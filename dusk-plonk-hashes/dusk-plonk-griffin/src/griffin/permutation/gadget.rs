...
use super::Griffin;
use crate::griffin::{CONSTANTS_ALPHA_BETA, CONSTANTS_AFFINE, FIVE_INV, MATRIX, WIDTH};


/// An implementation for the Griffin permutation operating on [`Witness`]es.
...
impl<'a> Griffin<Witness> for GadgetPermutation<'a> {
    ...
    fn quintic_s_box_inv(&mut self, value: &mut Witness) {
        let witness_x = value.clone();
        let value_x = self.composer[witness_x];
        let value_y = value_x.pow_vartime(&FIVE_INV);
        ...
        // y^5
        let constraint = Constraint::new().mult(1).a(witness_y_4).b(witness_y).fourth(-BlsScalar::one()).d(witness_x);
        *value = self.composer.gate_mul(constraint);
    ...
    fn horst(&mut self, round: usize, state: &mut [Witness; WIDTH]) {
        ...
        let constraint = Constraint::new()
            ...
            .right(1)
            .b(state[1]);
        let l_2 = self.composer.gate_add(constraint);
        let constraint = Constraint::new()
            .mult(1)
            .a(l_2)
            .b(l_2)
            .right(CONSTANTS_ALPHA_BETA[round][0][0])
            .constant(CONSTANTS_ALPHA_BETA[round][0][1]);
        let squ_2 = self.composer.gate_mul(constraint);
        let constraint = Constraint::new()
            .mult(1)
            .a(state[2])
            .b(squ_2);
        state[2] = self.composer.gate_mul(constraint);
        
        for i in 3..WIDTH {
            let constraint = Constraint::new()
                .left(BlsScalar::from_raw([(i - 1) as u64, 0, 0, 0]))
                .a(state[0])
                .right(1)
                .b(state[1])
                .fourth(1)
                .d(state[i - 1]);
            let l_i = self.composer.gate_add(constraint);
            let constraint = Constraint::new()
                .mult(1)
                .a(l_i)
                .b(l_i)
                .right(CONSTANTS_ALPHA_BETA[round][i - 2][0])
                .constant(CONSTANTS_ALPHA_BETA[round][i - 2][1]);
            let squ_i = self.composer.gate_mul(constraint);
            let constraint = Constraint::new()
                .mult(1)
                .a(state[i])
                .b(squ_i);
...
mod tests {
    ...
    use crate::griffin::ScalarPermutation;
    ...
    impl Circuit for TestCircuit {
        fn circuit(&self, composer: &mut Composer) -> Result<(), Error> {
            ...
            // Apply Griffin gadget permutation.
